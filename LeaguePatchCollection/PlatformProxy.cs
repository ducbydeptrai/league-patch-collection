...
public partial class PlatformProxy
{
    private TcpListener? _listener;
    private CancellationTokenSource? _cts;
    private static readonly string[] separator = ["\r\n"];
    private static string? loginQueueResponseJson;
    ...
    private static async Task HandleClientAsync(TcpClient client, CancellationToken token)
    {
        ...
        try
        {
            ...
            string? targetHost = ConfigProxy.PlatformUrl?.Replace("https://", "");
    ...
    {
        if (endpoint.StartsWith("/login-queue/v2/login/products/lol/regions/"))
        {
            try
            {
                string responseJson = Encoding.UTF8.GetString(payload);
                loginQueueResponseJson = responseJson; // Store the entire response globally
            ...
            catch (Exception ex)
            {
                ...
                Console.WriteLine($"[ERROR] Failed to parse login queue response: {ex.Message}");
            }
        }
