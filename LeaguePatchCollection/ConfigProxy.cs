...
class HttpProxy
{
    internal sealed class ConfigController : WebApiController
    {
        ...
        public async Task GetConfigPlayer()
        {
            ...
            if (HttpContext.Request.Url.LocalPath == "/api/v1/config/public")
            {
                ...
                if (SettingsManager.ConfigSettings.Novgk)
                {
                ...
                if (SettingsManager.ConfigSettings.Legacyhonor)
                {
                ...
                if (SettingsManager.ConfigSettings.Namebypass)
                {
                ...
                if (SettingsManager.ConfigSettings.Nobloatware)
                {
                ...
                if (SettingsManager.ConfigSettings.Oldpatch)
                {
    ...
    internal sealed class LedgeController : WebApiController
    {
        ...
        public async Task GetLedge()
        {
            ...
            if (SettingsManager.ConfigSettings.Nobehavior)
            {
    ...
    internal sealed class GeopassController : WebApiController
    {
        ...
        private async Task SendResponse(HttpResponseMessage response, string content)
        {
            ...
            if (response.StatusCode == HttpStatusCode.Forbidden)
            {
                Trace.WriteLine("Geopass request Cloudflare blocked (403), please open issue on GitHub or contact c4t_bot on Discord");
            }
    ...
    internal sealed class httpProxyServer<T> where T : WebApiController, new()
    {
        ...
        {
            // Ensure the cancellation token is passed to stop the server if requested
    ...
    private static void StartBackgroundTaskForRmsAffinities(JsonObject rmsAffinities)
    {
        Task.Run(() =>
        {
            Trace.WriteLine("Received RMS Affinities:");
            ...
            {
                Trace.WriteLine($"RMS Host for user region '{userRegionRms}': {tempRmsHost}");
    ...
    private static void StartBackgroundTaskForChatAffinities(JsonObject chatAffinities)
    {
        Task.Run(() =>
        {
            Trace.WriteLine("Received Chat Affinities:");
            ...
            {
                Trace.WriteLine($"Chat Host for user region '{userRegion}': {tempChatHost}");
