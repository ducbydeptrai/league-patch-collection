...
namespace LeaguePatchCollection
{
    public partial class XMPPProxy
    {
        ...
        public async Task RunAsync(CancellationToken token)
        {
            ...
            {
                Console.WriteLine($"[XMPP] Error in listener: {ex.Message}");
            }
            finally
            {
                _listener?.Stop();
        ...
        private async Task ForwardDataAsync(Stream source, Stream destination, string direction, CancellationToken token)
        {
            ...
            catch (Exception ex)
            {
                if (!token.IsCancellationRequested)
                {
                    Trace.WriteLine($"[XMPP] Error during {direction}: {ex.Message}");
                }
            }
        }
        ...
        public void Stop()
        {
            try
            {
                if (_cts == null)
                {
                    Trace.WriteLine("[XMPP] Proxy service is not running.");
                    return;
                }
                ...
                {
                    Trace.WriteLine("[XMPP] Listener is null, cannot stop proxy.");
                }

                Trace.WriteLine("[XMPP] Stopping proxy...");
            }
            catch (Exception ex)
            {
                // Catch any exceptions and log them
                Trace.WriteLine($"[XMPP] Error stopping proxy: {ex.Message}");
            }
        }
